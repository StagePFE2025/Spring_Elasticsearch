...
public class ElasticsearchConfig2 {
    ...
    public RestClient restClient() throws Exception {
        ...
        try (InputStream is = getClass().getClassLoader().getResourceAsStream("elastic-stack-ca.p12")) {
            trustStore.load(is, "alouan01".toCharArray());
        }

        // Charger le keystore pour l'authentification client
        KeyStore keyStore = KeyStore.getInstance("PKCS12");
        try (InputStream is = getClass().getClassLoader().getResourceAsStream("elastic-certificates.p12")) {
        ...
        credentialsProvider.setCredentials(AuthScope.ANY,
                new UsernamePasswordCredentials("elastic", "alouan01")); // <= Remplace si besoin
        //D6YHB8A8DQCEb73qeXSt  pour l'utilisateur elastic localhost
        //=UA9yH=of265upUALcUB pour l'utilisateur elastic sur le serveur distant

        // Désactiver seulement la vérification du nom d'hôte
        HostnameVerifier hostnameVerifier = new NoopHostnameVerifier();

        // Créer le client REST
        return RestClient.builder(new HttpHost("13.36.113.158", 9200, "https"))
                .setHttpClientConfigCallback(httpClientBuilder -> httpClientBuilder
                        ...
                        .setKeepAliveStrategy((response, context) -> 60000L)
                )
                ...
                .setFailureListener(new RestClient.FailureListener() {
                    ...
                    public void onFailure(Node node) {
                        System.err.println("Échec de la connexion au nœud : " + node);
                    }
                })
                .build();
    }
